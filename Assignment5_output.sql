...
SQL> 
SQL> 
SQL> @"C:\Rohith\Backup\Desktop\SEM 4\Database lab\Exercise-5\queries.sql"
...
SQL> 
SQL> CREATE OR REPLACE PROCEDURE model_check
  2  (model_input IN CAR_NAMES.Model%TYPE)
  3  IS
  4    CURSOR c1 IS
  5  	 SELECT cm.fullname, c.countryname
  6  	 FROM CAR_NAMES cn
  7  	 JOIN MODEL_DETAILS md ON cn.model = md.model
  8  	 JOIN CAR_MAKERS cm ON md.maker = cm.id
  9  	 JOIN COUNTRIES c ON cm.country = c.countryid
 10  	 WHERE cn.model = model_input;
 11  
 12    fullname CAR_MAKERS.Fullname%TYPE;
 13    country COUNTRIES.CountryName%TYPE;
 14  BEGIN
 15    OPEN c1;
 16    FETCH c1 INTO fullname,country;
 17    IF c1%FOUND THEN
 18  	 DBMS_OUTPUT.PUT_LINE('------------------------------------------------------');
 19  	 DBMS_OUTPUT.PUT_LINE('MODEL ' || model_input || ' IS MANUFACTURED');
 20  	 DBMS_OUTPUT.PUT_LINE('Maker Name: ' || fullname);
 21  	 DBMS_OUTPUT.PUT_LINE('Country: ' || country);
 22    ELSE
 23  	 DBMS_OUTPUT.PUT_LINE('The given model is not manufactured / Invalid Model');
 24    END IF;
 25    CLOSE c1;
 26  END;
 27  /
...
SQL> 
SQL> EXECUTE model_check('&model_input');
Enter value for model_input: mercedes
------------------------------------------------------                                                                            
...
SQL> 
SQL> CREATE OR REPLACE PROCEDURE desired_mileage
  2  (input_mileage IN CAR_DETAILS.MPG%TYPE)
  3  IS
  4    CURSOR c1 IS
  5  	 SELECT cd.Id,cn.Model,cn.MakeDescription,cd.MPG
  6  	 FROM CAR_DETAILS cd
  7  	 JOIN CAR_NAMES cn ON cd.Id=cn.Id
  8  	 ORDER BY ABS(cd.MPG-input_mileage);
  9  
 10    car_number CAR_DETAILS.Id%TYPE;
 11    model CAR_NAMES.Model%TYPE;
 12    description CAR_NAMES.MakeDescription%TYPE;
 13    mileage CAR_DETAILS.MPG%TYPE;
 14  BEGIN
 15    OPEN c1;
 16    FETCH c1 INTO car_number,model,description,mileage;
 17  	 DBMS_OUTPUT.PUT_LINE('CARID   MODEL	 CARNAME	    MILEAGE');
 18  	 DBMS_OUTPUT.PUT_LINE('-------------------------------------------------------');
 19  	 DBMS_OUTPUT.PUT_LINE(car_number || '	 ' || model || '   ' || description || '   ' || mileage);
...
SQL> 
SQL> EXEC desired_mileage(&wanted_mileage);
Enter value for wanted_mileage: 45
CARID   MODEL     CARNAME            MILEAGE                                                                                      
-------------------------------------------------------                                                                           
...
SQL> 
SQL> Rem Creation of procedure:
SQL> CREATE OR REPLACE PROCEDURE CAR_COUNT_BY_COUNTRY(cname IN COUNTRIES.countryname%TYPE) IS
  2    country_exists NUMBER := 0;
  3    cMaker CAR_MAKERS.maker%TYPE;
  4    cFname CAR_MAKERS.fullname%TYPE;
  5    cnt NUMBER;
  6    total_cars NUMBER := 0;
  7  BEGIN
  8    SELECT COUNT(*) INTO country_exists FROM COUNTRIES WHERE countryname = cname;
  9  
 10    IF country_exists = 0 THEN
 11  	 DBMS_OUTPUT.PUT_LINE('COUNTRY NOT IN DATABASE');
 12  	 RETURN;
 13    END IF;
 14  
 15    DBMS_OUTPUT.PUT_LINE('Maker Name 	Model	  No. of cars');
 16    DBMS_OUTPUT.PUT_LINE('---------------------------------------------');
 17  
 18    FOR car IN (
 19  	 SELECT cm.maker, md.model, COUNT(*) AS num_cars
 20  	 FROM CAR_MAKERS cm, MODEL_DETAILS md, CAR_NAMES cn, CAR_DETAILS cd, COUNTRIES c
 21  	 WHERE cm.id = md.maker AND cm.country = c.countryid AND md.model = cn.model AND cn.id = cd.id AND c.countryname = cname
 22  	 GROUP BY cm.maker, md.model
 23  	 ORDER BY cm.maker, md.model
 24    ) LOOP
 25  	 cMaker := car.maker;
 26  	 cFname := car.model;
 27  	 cnt := car.num_cars;
 28  	 DBMS_OUTPUT.PUT_LINE(cmaker||' 	  '||cFname||'	      '||cnt);
 29  	 total_cars := total_cars + cnt;
 30    END LOOP;
 31  
 32    IF total_cars = 0 THEN
 33  	 DBMS_OUTPUT.PUT_LINE('The country '||cname||' does not produce any car');
 34  	 DBMS_OUTPUT.PUT_LINE('---------------------------------------------');
 35  	 DBMS_OUTPUT.PUT_LINE('Total : '||total_cars);
 36    ELSE
 37  	 DBMS_OUTPUT.PUT_LINE('---------------------------------------------');
 38  	 DBMS_OUTPUT.PUT_LINE('Total : ' || total_cars);
 39    END IF;
 40  END;
 41  /
