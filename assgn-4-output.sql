SQL> 
SQL> @"C:\Rohith\Backup\Desktop\SEM 4\Database lab\Exercise-4\creation.sql"
SQL> set long 50000
...
SQL> 
SQL> 
SQL> @"C:\Rohith\Backup\Desktop\SEM 4\Database lab\Exercise-4\queries.sql"
SQL> REM ----------------------------- Queries----------------------------
> 
SQL> REM 1. Create a view named Datsun_Cars, which display the car id, model and descriptions of Datsun model
SQL> 
SQL> CREATE OR REPLACE VIEW Datsun_Cars
  2  AS
  3  SELECT Id,Model,MakeDescription
  4  FROM CAR_NAMES
  5  WHERE Model='datsun';
...
SQL> 
SQL> 
SQL> REM Updatable or not? -Updatable because of the presence of the primary key in the view
SQL> 
SQL> UPDATE Datsun_Cars
  2  SET MakeDescription='datsun V12 engine'
  3  WHERE Id=371;

1 row updated.

SQL> 
SQL> REM Insertion accepted
SQL> 
SQL> INSERT INTO Datsun_Cars(Id,Model,MakeDescription)
  2  VALUES (407,'datsun','datsun V8 engine');

1 row created.

SQL> 
SQL> REM integrity constraint (01085.ID_FK) violated - child record found
SQL> 
SQL> DELETE FROM Datsun_Cars
  2  WHERE Id=394;
DELETE FROM Datsun_Cars
*
ERROR at line 1:
ORA-02292: integrity constraint (B.ID_FK) violated - child record found 
...
---------- --------------- ----------------------------------------                                                               
       ...
       355 datsun          datsun 210 mpg                                                                                         
       365 datsun          datsun 200sx                                                                                           
       371 datsun          datsun V12 engine                                                                                      

        ID MODEL           MAKEDESCRIPTION                                                                                        
---------- --------------- ----------------------------------------                                                               
       394 datsun          datsun 310 gx                                                                                          
       407 datsun          datsun V8 engine                                                                                       

24 rows selected.

SQL> 
SQL> 
SQL> REM Displaying Base Table
SQL> 
SQL> SELECT Id,Model,MakeDescription
  2  FROM CAR_NAMES
  3  WHERE Model='datsun';
...
SQL> REM -----------------------------------------------------------------------------------------------------------
> 
SQL> REM 2. Create a view called Car_List that shows the car id, model, description and operational parameters of all cars produced during 1974. Make surethat, the year should not be reassigned to any other value through view.
SQL> 
SQL> CREATE OR REPLACE VIEW Car_List
  2  AS
  3  SELECT cn.Id,cn.Model,cn.MakeDescription,cd.MPG,cd.Cylinders,cd.Edispl,cd.Horsepower,cd.Accelerate
  4  FROM CAR_NAMES cn JOIN CAR_dETAILS cd
  5  ON cd.Id=cn.Id
  6  WHERE Year=1974;
...
SQL> 
SQL> 
SQL> REM Dsiplaying View
SQL> 
SQL> SELECT * FROM Car_LIst;

        ID MODEL           MAKEDESCRIPTION                                 MPG  CYLINDERS     EDISPL HORSEPOWER ACCELERATE        
---------- --------------- ---------------------------------------- ---------- ---------- ---------- ---------- ----------        
       133 plymouth        plymouth duster                                  20          6        198         95       16.5        
       134 ford            ford maverick                                    21          6        200                    17        
       135 amc             amc hornet                                       19          6        232        100         16        
       136 chevrolet       chevrolet nova                                   15          6        250        100         17        
       137 datsun          datsun b210                                      31          4         79         67         19        
       138 ford            ford pinto                                       26          4        122         80       16.5        
       139 toyota          toyota corolla 1200                              32          4         71         65         21        
       140 chevrolet       chevrolet vega                                   25          4        140         75         17        
       141 chevrolet       chevrolet chevelle malibu classic                16          6        250        100         17        
       142 amc             amc matador                                      16          6        258        110         18        
       143 plymouth        plymouth satellite sebring                       18          6        225        105       16.5        

        ID MODEL           MAKEDESCRIPTION                                 MPG  CYLINDERS     EDISPL HORSEPOWER ACCELERATE        
---------- --------------- ---------------------------------------- ---------- ---------- ---------- ---------- ----------        
       144 ford            ford gran torino                                 16          8        302        140         14        
       145 buick           buick century luxus (sw)                         13          8        350        150       14.5        
       146 dodge           dodge coronet custom (sw)                        14          8        318        150       13.5        
       147 ford            ford gran torino (sw)                            14          8        302        140         16        
       148 amc             amc matador (sw)                                 14          8        304        150       15.5        
       149 audi            audi fox                                         29          4         98         83       16.5        
       150 volkswagen      volkswagen dasher                                26          4         79         67       15.5        
       151 opel            opel manta                                       26          4         97         78       14.5        
       152 toyota          toyota corona                                    31          4         76         52       16.5        
       153 datsun          datsun 710                                       32          4         83         61         19        
       154 dodge           dodge colt                                       28          4         90         75       14.5        

        ID MODEL           MAKEDESCRIPTION                                 MPG  CYLINDERS     EDISPL HORSEPOWER ACCELERATE        
---------- --------------- ---------------------------------------- ---------- ---------- ---------- ---------- ----------        
       155 fiat            fiat 128                                         24          4         90         75       15.5        
       156 fiat            fiat 124 tc                                      26          4        116         75         14        
       157 honda           honda civic                                      24          4        120         97         15        
       158 subaru          subaru                                           26          4        108         93       15.5        
       159 fiat            fiat x1.9                                        31          4         79         67         16        

27 rows selected.

SQL> 
SQL> 
SQL> REM Updatable or Not? - NOT UPDATABLE cannot modify a column which maps to a non key-preserved table
SQL> 
SQL> UPDATE Car_List
  2  SET Model='new_model'
  3  WHERE Id=159;
SET Model='new_model'
    *
ERROR at line 2:
...
SQL> 
SQL> REM Key preserved means the row from the base table will appear AT MOST ONCE in the output view on that table.
SQL> REM cannot modify a column which maps to a non key-preserved table
SQL> 
SQL> INSERT INTO Car_List(Id,Model,MakeDescription,MPG,Cylinders,Edispl,Horsepower,Accelerate)
  2  VALUES(408,'ford','F-150 Lightning',11,8,250,220,21);
INSERT INTO Car_List(Id,Model,MakeDescription,MPG,Cylinders,Edispl,Horsepower,Accelerate)
                     *
ERROR at line 1:
ORA-01779: cannot modify a column which maps to a non key-preserved table 
...
SQL> 
SQL> REM Value will be deleted
SQL> 
SQL> DELETE FROM Car_List
  2  WHERE MakeDescription='fiat x1.9';
...
SQL> REM Displaying Base Table
SQL> 
SQL> SELECT cn.Id,cn.Model,cn.MakeDescription,cd.MPG,cd.Cylinders,cd.Edispl,cd.Horsepower,cd.Accelerate
  2  FROM CAR_NAMES cn JOIN CAR_dETAILS cd
  3  ON cd.Id=cn.Id
  4  WHERE Year=1974;
...
SQL> REM -----------------------------------------------------------------------------------------------------------
> 
SQL> REM 3. Create a view named European_Makers that will display the makername, full name and the country name of car makersfrom Europe
SQL> 
SQL> CREATE OR REPLACE VIEW European_Makers
  2  AS
  3  SELECT cm.Id,cm.Fullname,c.CountryName
  4  FROM Continents cont JOIN countries c
  5  ON cont.ContId=c.Continent
  6  JOIN CAR_MAKERS cm
  7  ON c.CountryId=cm.Country
  8  WHERE cont.Continent='europe';
...
SQL> SELECT * FROM European_Makers;
        ...
        ID FULLNAME                       COUNTRYNAME                                                                             
---------- ------------------------------ --------------------                                                                    
         2 Volkswagen                     germany                                                                                 
         3 BMW                            germany                                                                                 
         7 Citroen                        france                                                                                  
         9 Fiat                           italy                                                                                   
        13 Daimler Benz                   germany                                                                                 
        14 Opel                           germany                                                                                 
        15 Peugeaut                       france                                                                                  
        16 Renault                        france                                                                                  
        17 Saab                           sweden                                                                                  
...
SQL> 
SQL> 
SQL> REM Updatable or Not? Updatable
SQL> REM If there is atleast one or more primary key for updatable.
SQL> 
SQL> UPDATE European_Makers
  2  SET FullName='British Motor Works'
  3  WHERE Id=3;

1 row updated.

SQL> 
SQL> REM cannot modify more than one base table through a join view
SQL> 
SQL> INSERT INTO European_Makers(Maker,FullName,CountryName)
  2  VALUES ('Mercedes','S480 Maybach','germany');
INSERT INTO European_Makers(Maker,FullName,CountryName)
                            *
ERROR at line 1:
ORA-00904: "MAKER": invalid identifier 


SQL> 
SQL> REM integrity constraint (01085.MAKER_FK) violated - child record found
SQL> 
SQL> DELETE FROM European_Makers
  2  WHERE Maker='volvo';
WHERE Maker='volvo'
      *
ERROR at line 2:
...
SQL> REM Displaying Base Table
SQL> 
SQL> SELECT cm.Maker,cm.Fullname,c.CountryName
  2  FROM Continents cont JOIN countries c
  3  ON cont.ContId=c.Continent
  4  JOIN CAR_MAKERS cm
  5  ON c.CountryId=cm.Country
  6  WHERE cont.Continent='europe';

MAKER           FULLNAME                       COUNTRYNAME                                                                        
--------------- ------------------------------ --------------------                                                               
volkswagen      Volkswagen                     germany                                                                            
bmw             British Motor Works            germany                                                                            
citroen         Citroen                        france                                                                             
fiat            Fiat                           italy                                                                              
daimler benz    Daimler Benz                   germany                                                                            
opel            Opel                           germany                                                                            
peugeaut        Peugeaut                       france                                                                             
...
SQL> REM -----------------------------------------------------------------------------------------------------
> 
SQL> REM 4. Create a view named Cars_Count which displays total number of cars manufactured by each country
SQL> 
SQL> CREATE OR REPLACE VIEW Cars_Count
  2  AS
  3  SELECT COUNT(cm.Id) AS "COUNT",c.CountryName
  4  FROM CAR_MAKERS cm
  5  JOIN Countries c ON c.CountryId=cm.Country
  6  GROUP BY c.CountryName;
...
SQL> 
SQL> 
SQL> REM Updatable or not? Not Updatable - GROUP BY is used and also COUNT(cm.Id) is creating a new column(virtual column)
SQL> REM and hence it is not updatable
SQL> 
SQL> UPDATE Cars_Count
  2  SET COUNT=10
...
SQL> 
SQL> REM virtual column not allowed here
SQL> REM Virtual column - When queried, virtual columns look like normal table columns,
SQL> REM but their values are derived rather than being stored on disc
SQL> 
SQL> INSERT INTO Cars_Count(Count,CountryName)
  2  VALUES(5,'australia');
INSERT INTO Cars_Count(Count,CountryName)
                       *
ERROR at line 1:
...
SQL> 
SQL> 
SQL> REM data manipulation operation not legal on this view - GROUP BY
SQL> 
SQL> DELETE FROM Cars_Count
  2  WHERE CountryName='france';
...
SQL> REM Displaying Base Table
SQL> 
SQL> SELECT COUNT(cm.Id) AS "COUNT",c.CountryName
  2  FROM CAR_MAKERS cm
  3  JOIN Countries c ON c.CountryId=cm.Country
  4  GROUP BY c.CountryName;
