SQL> @"C:\Rohith\Backup\Desktop\SEM 4\Database lab\Exercise-5\creation.sql"
SQL> set long 50000
...
SQL> @"C:\Rohith\Backup\Desktop\SEM 4\Database lab\Exercise-5\queries_anonymous_blocks.sql"
SQL> SET SERVEROUTPUT ON;
SQL> 
SQL> REM CHR(10) is used to insert line breaks, CHR(9) is for tabs
SQL> 
SQL> REM 1 Check whether the given model is manufactured by any maker. If available, display the maker full name and country else display: "The given model is not manufactured / Invalid Model"
SQL> 
SQL> declare
  2    mName MODEL_DETAILS.model%TYPE;
  3    mFname CAR_MAKERS.fullname%TYPE;
  4    mCountry COUNTRIES.countryname%TYPE;
  5  
  6  begin
  7    mName := '&mName';
  8    select cm.fullname, c.countryname
  9    into mFname, mCountry
 10    from CAR_MAKERS cm
 11    join COUNTRIES c on c.countryid = cm.country
 12    join MODEL_DETAILS md on md.maker = cm.id
 13    where md.model = mName;
 14    if SQL%FOUND then
 15  	 dbms_output.put_line(chr(10)|| 'MODEL ' || mName || ' IS MANUFACTURED BY');
 16  	 dbms_output.put_line('Maker Name: ' || mFname);
 17  	 dbms_output.put_line('Country: ' || mCountry);
 18    end if;
 19  
 20  exception
 21    when NO_DATA_FOUND then
 22  	 dbms_output.put_line(chr(10) || 'The given model is not manufactured / Invalid Model');
 23  end;
 24  /
Enter value for mname: mercedes
old   7:   mName := '&mName';
new   7:   mName := 'mercedes';

MODEL mercedes IS MANUFACTURED BY                                                                                                
...
SQL> REM -----------------------------------------------------------------------------------
> 
SQL> REM 2 An user is desired to buy a car with the specific mileage. Ask the user for a mileage, and find the car that is equal or closest to the desired mileage.Print the car number, model, description and mileage. Also print the number of car(s) that is equal or closest to the given mileage.
SQL> 
SQL> declare
  2    cMile CAR_DETAILS.mpg%TYPE := '&cMile';
  3    cId CAR_DETAILS.id%TYPE;
  4    cModel CAR_NAMES.model%TYPE;
  5    cDescr CAR_NAMES.Makedescription%TYPE;
  6    cCount number(3);
  7    cursor car_cur is
  8  	 select cd.id, cn.model, cn.Makedescription, cd.mpg
  9  	 from CAR_DETAILS cd
 10  	 join CAR_NAMES cn on cn.id = cd.id
 11  	 where abs(cd.mpg - cMile) = (select min(abs(cd.mpg - cMile)) from CAR_DETAILS cd);
 12  
 13  begin
 14    dbms_output.put_line(chr(10));
 15    dbms_output.put_line('CARID' || chr(9) || 'MODEL' || chr(9) || 'DESCRIPTION' || chr(9) || 'MILEAGE');
 16  
 17    open car_cur;
 18    loop
 19  	 fetch car_cur into cId, cModel, cDescr, cMile;
 20  	 exit when car_cur%NOTFOUND;
 21  	 dbms_output.put_line(cId || chr(9) || cModel || chr(9) || cDescr || chr(9) || cMile);
 22    end loop;
 23  
 24    select count(*) into cCount
 25    from CAR_DETAILS
 26    where abs(mpg - cMile) = (select min(abs(mpg - cMile)) from CAR_DETAILS);
 27  
 28    dbms_output.put_line(chr(10) || cCount || ' car(s) found EQUAL/CLOSEST to given mileage');
 29  end;
 30  /
Enter value for cmile: 45
old   2:   cMile CAR_DETAILS.mpg%TYPE := '&cMile';
new   2:   cMile CAR_DETAILS.mpg%TYPE := '45';

                                                                                                                                 
CARID	MODEL	DESCRIPTION	MILEAGE                                                                                                   
...
SQL> REM ---------------------------------------------------------------------------------------
> 
SQL> REM 3 For a given country name, display the number of cars manufactured in each model by the car makers as shown below. Check for the availabilityof country name.
SQL> 
SQL> declare
  2    cName COUNTRIES.countryname%TYPE := '&cName';
  3    cMaker CAR_MAKERS.maker%TYPE;
  4    cFname CAR_MAKERS.fullname%TYPE;
  5    cnt number;
  6    cCount int :=0;
  7    total_cars int :=0;
  8    cursor car_cur is
  9  	 select cm.maker, md.model, COUNT(*) as num_cars
 10  	 from CAR_MAKERS cm
 11  	 join MODEL_DETAILS md on cm.id = md.maker
 12  	 join CAR_NAMES cn on md.model = cn.model
 13  	 join CAR_DETAILS cd on cn.id = cd.id
 14  	 join COUNTRIES c on cm.country = c.countryid
 15  	 where c.countryname = cName
 16  	 group by cm.maker, md.model;
 17  
 18  begin
 19    select count(*)
 20    into cCount
 21    from COUNTRIES
 22    where countryname = cName;
 23  
 24    if cCount = 0 then
 25  	 dbms_output.put_line(chr(10) || 'COUNTRY NOT IN DATABASE');
 26  	 return;
 27    end if;
 28  
 29    dbms_output.put_line(chr(10));
 30  	     dbms_output.put_line('Country Name: ' || cName);
 31  	     dbms_output.put_line('Maker Name ' || 'Model ' || 'NumOfCars');
 32  
 33    for car_rec in car_cur
 34    loop
 35  	 cMaker := car_rec.maker;
 36  	 cFname := car_rec.model;
 37  	 cnt := car_rec.num_cars;
 38  	 total_cars := total_cars + cnt;
 39  	 dbms_output.put_line(cMaker || '    ' || cFname || '	 ' || cnt);
 40    end loop;
 41  
 42    select sum(totCars)
 43    into cCount
 44    from (
 45  	 select count(*) as totCars
 46  	 from CAR_MAKERS cm
 47  	 join MODEL_DETAILS md on cm.id = md.maker
 48  	 join CAR_NAMES cn on md.model = cn.model
 49  	 join CAR_DETAILS cd on cn.id = cd.id
 50  	 join COUNTRIES c on cm.country = c.countryid
 51  	 where c.countryname = cName
 52  	 group by cm.maker, md.model
 53    );
 54  
 55    IF total_cars = 0 THEN
 56  	 dbms_output.put_line(chr(10));
 57  	 DBMS_OUTPUT.PUT_LINE('The country '||cname||' does not produce any car');
 58  	 DBMS_OUTPUT.PUT_LINE('---------------------------------------------');
 59  	 DBMS_OUTPUT.PUT_LINE('Total : '||total_cars);
 60    ELSE
 61  	 DBMS_OUTPUT.PUT_LINE('---------------------------------------------');
 62  	 DBMS_OUTPUT.PUT_LINE('Total : ' || total_cars);
 63    END IF;
 64  
 65  end;
 66  /
Enter value for cname: korea
old   2:   cName COUNTRIES.countryname%TYPE := '&cName';
new   2:   cName COUNTRIES.countryname%TYPE := 'korea';

                                                                                                                                 
Country Name: korea                                                                                                               
Maker Name Model NumOfCars                                                                                                        
...
SQL> /
Enter value for cname: Isreal
old   2:   cName COUNTRIES.countryname%TYPE := '&cName';
new   2:   cName COUNTRIES.countryname%TYPE := 'Isreal';
...
SQL> /
Enter value for cname: france
old   2:   cName COUNTRIES.countryname%TYPE := '&cName';
new   2:   cName COUNTRIES.countryname%TYPE := 'france';
